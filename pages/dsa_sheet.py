...
def draw():
    ...
    # Initial predefined schedule (your original given schedule)
    INITIAL_SCHEDULE = [
    ...
    # Format start/end dates to string range e.g. 'Jul 23 – Jul 29'
    def format_date_range(start, end):
        return f"{start.strftime('%b %d')} – {end.strftime('%b %d')}"

    # Insert an entry and shift the schedule respecting fixed breaks
    def insert_and_shift_schedule(name, start, end, type_, note=""):
        ...
        # Recalculate dates, skipping fixed blocks (fixed blocks keep original dates)
        recalculated = []
        cursor = start
        for item in new_schedule:
            ...
            if is_fixed:
                ...
                cursor = parse_start_date(item["Date Range"]).date() + datetime.timedelta(days=days)
                continue
    ...
    # --- Input Bars Below Table ---
    
    # Green Bar - Study Topic
    st.markdown("<div style='background:#e9ffe9;padding:15px;margin-top:15px;border-radius:8px;'>"
                "<h4>🟩 Add Study Topic</h4>", unsafe_allow_html=True)
    g_col1, g_col2 = st.columns(2)
    green_topic = g_col1.text_input("Topic You Studied / Completed", key="green_topic")
    green_from = g_col2.date_input("From Date", key="green_from", value=today)
    green_to = g_col2.date_input("Till Date", key="green_to", value=today + datetime.timedelta(days=1))
    if st.button("➕ GO - Add Study Topic"):
    ...
    st.markdown("<div style='background:#ffeaea;padding:15px;margin-top:15px;border-radius:8px;'>"
                "<h4>🟥 Add Fun Activity</h4>", unsafe_allow_html=True)
    r_col1, r_col2 = st.columns(2)
    red_topic = r_col1.text_input("Fun You Did (e.g. Movie, Sports)", key="red_topic")
    red_from = r_col2.date_input("From Date", key="red_from", value=today)
    red_to = r_col2.date_input("Till Date", key="red_to", value=today + datetime.timedelta(days=1))
    if st.button("➕ GO - Add Fun Activity"):
    ...
    st.markdown("<div style='background:#f0f0f0;padding:15px;margin-top:15px;border-radius:8px;'>"
                "<h4>⬜ Add Wasted Time</h4>", unsafe_allow_html=True)
    y_col1, y_col2 = st.columns(2)
    gray_reason = y_col1.text_input("Wasted Time Reason", key="gray_reason")
    gray_from = y_col2.date_input("From Date", key="gray_from", value=today)
    gray_to = y_col2.date_input("Till Date", key="gray_to", value=today + datetime.timedelta(days=1))
